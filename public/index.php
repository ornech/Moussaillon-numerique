            ...
            if ($role === 'eleve') {
                ...
                if (!$user) {
                    ...
                    if (preg_match('/^\d{4}$/', $password)) {
                        ...
                        \Jf\Moussaillons\Application\Auth\SessionCreator::create((int)$pdo->lastInsertId(), 'eleve', $username);
                    } else {
                        $message = "Le PIN doit contenir exactement 4 chiffres.";
                    }
                } else {
                    if (password_verify($password, $user['pin_hash'])) {
                        \Jf\Moussaillons\Application\Auth\SessionCreator::create((int)$user['id'], 'eleve', $username);
                    } else {
                ...
                if ($staff && password_verify($password, $staff['pin_hash']) && $staff['role'] === $role) {
                    \Jf\Moussaillons\Application\Auth\SessionCreator::create((int)$staff['id'], $staff['role'], $username);
                } else {
